...
class LabelEncoder:
    ...
    def get_one_hot_encoded_labels_df(self):
        ...
        return self.drop_columns(one_hot_encoded_genres)

    def drop_columns(self, one_hot_encoded_genres):
        one_hot_encoded_genres = one_hot_encoded_genres.drop(columns=self.genres_to_drop)
        genre_cols = one_hot_encoded_genres.columns.drop('track_id')

        return one_hot_encoded_genres[one_hot_encoded_genres[genre_cols].sum(axis=1) > 0]

    def get_valid_track_genre_pairs(self):
        fma_subset_metadata = self.get_subset_tracks_metadata_df()
        all_track_genre_pairs = self.get_all_track_genre_pairs(fma_subset_metadata)
